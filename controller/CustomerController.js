...
const createCustomer = async (req, resp) => {
    try {
        const {name, address, salary, contact} = req.body;//catch customer data
        const createdCustomer = new CustomerSchema({
            name, address, salary, contact
        });//create customer object
        await createdCustomer.save();//save the customer
        resp.status(201).json({message: 'Customer Saved...'});//save response
